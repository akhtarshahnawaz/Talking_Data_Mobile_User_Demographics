...
ltrain  = np.matrix(np.arange(0,appLabelDeviceTrain.shape[0])/float(appLabelDeviceTrain.shape[0])).reshape(appLabelDeviceTrain.shape[0],1)
train = hstack((appLabelDeviceTrain,areaTrain,ltrain), format='csr')

appLabelDeviceTest = pickle.load(open("../generated/appLabelDeviceTest.p","r"))
areaTest = pickle.load(open("../generated/areaTest.p","r"))
ltest  = np.matrix(np.arange(0,appLabelDeviceTest.shape[0])/float(appLabelDeviceTest.shape[0])).reshape(appLabelDeviceTest.shape[0],1)
test = hstack((appLabelDeviceTest,areaTest,ltest), format='csr')
