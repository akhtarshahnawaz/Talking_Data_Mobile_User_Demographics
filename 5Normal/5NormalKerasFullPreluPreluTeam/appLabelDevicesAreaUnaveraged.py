...
def runKeras(x_train,x_test,y_train,y_test):
	clf = modelBuilder()
	fit= clf.fit_generator(generator=batch_generator(x_train, y_train, 100, True),nb_epoch=3,samples_per_epoch=(train.shape[0]/100)*100,validation_data=(x_test.todense(), y_test), verbose=2)
...
clf = modelBuilder()
fit= clf.fit_generator(generator=batch_generator(train, target, 100, True),nb_epoch=3,samples_per_epoch=(train.shape[0]/100)*100, verbose=2)
