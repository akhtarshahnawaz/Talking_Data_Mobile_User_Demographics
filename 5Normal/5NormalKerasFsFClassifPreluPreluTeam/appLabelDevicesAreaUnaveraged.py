...
def modelBuilder():
	model = Sequential()
	model.add(Dense(150, input_dim=train.shape[1], init='normal'))
	model.add(PReLU())
...
def runKeras(x_train,x_test,y_train,y_test):
	clf = modelBuilder()
	fit= clf.fit_generator(generator=batch_generator(x_train, y_train, 100, True),nb_epoch=4,samples_per_epoch=(train.shape[0]/100)*100,validation_data=(x_test.todense(), y_test), verbose=2)
...
for i in range(0,5):
	...
	cv_y_test = runKeras(x_train,x_test,y_train,y_test)
...
clf = modelBuilder()
fit= clf.fit_generator(generator=batch_generator(train, target, 100, True),nb_epoch=4,samples_per_epoch=(train.shape[0]/100)*100, verbose=2)
testStack = clf.predict_generator(generator=batch_generatorp(test, 400, False), val_samples=test.shape[0])
...
trainStack.to_csv("stacks/train/appLabelDevicesAreaUnaveraged.csv", index=False)
testStack.to_csv("stacks/test/appLabelDevicesAreaUnaveraged.csv", index=False)

#Generating Report for this Model
f = open("reports/appLabelDevicesAreaUnaveraged.txt","wb")
