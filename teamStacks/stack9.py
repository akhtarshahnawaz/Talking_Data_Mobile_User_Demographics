...
def runKeras(x_train,x_test,y_train,y_test,test):
	...
	tmpModel = "stack9.hdf5"
	early_stopping = EarlyStopping(monitor='val_loss', patience=5, verbose =1)
	checkpointer = ModelCheckpoint(filepath= tmpModel , monitor='val_loss', verbose=1, save_best_only=True)

	fit= clf.fit(x_train, y_train,nb_epoch=100,validation_data=(x_test, y_test), verbose=2,callbacks=[early_stopping,checkpointer])

	clf = load_model(tmpModel)

	cv_y_test = clf.predict(x_test)
	real_y_test = clf.predict(test)
...
trainStack.to_csv("stacks/train/stack9.csv", index=False)
testStack.to_csv("stacks/test/stack9.csv", index=False)

#Generating Report for this Model
f = open("reports/stack9.txt","wb")
